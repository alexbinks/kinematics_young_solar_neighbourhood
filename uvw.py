...
t = int(input('Working on the following file: ' + input_file + '\n...is this correct?\n(1) yes, (2) no'))
if t == 1:
    print("ok, let's go!")
if t != 1:
    print("ok, sayonnara")
...
input_dir = './data/'
data_dir = './data/'
output_file = input_file[:-4]+'_uvw.csv'

file_in   = input_dir + input_file

file_test = Path(data_dir + output_file)
if file_test.is_file() == True:
    t = int(input("Warning! There is already a file called" + output_file + "\nShall I overwrite this?\n(1) Yes, (2) No"))
    if t == 1:
        os.remove(data_dir + output_file)
#        file_in = input_dir + input_file
    if t == 2:
        print("OK, you got it, see you soon!")
        exit()
...
# CHECK COLUMN NAMES #
######################
dataset = pd.read_csv(file_in, usecols=['source_id', 'ra', 'dec', 'pmra', 'pmra_error', 'parallax', 'parallax_error', 'pmdec', 'pmdec_error', 'pmra_pmdec_corr', 'parallax_pmra_corr', 'parallax_pmdec_corr', 'radial_velocity', 'radial_velocity_error'])
...
print('Using theta = %.0f, delta = %.1f, alpha = %.2f' % (theta, delta, alpha))
